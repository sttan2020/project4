    ...
    public class ProfileService : IProfileService
    {
        ...
        public async Task<IEnumerable<TalentSnapshotViewModel>> GetTalentSnapshotList(string employerOrJobId, bool forJob, int position, int increment)
        {
            if (!forJob)
            {
                ...
                var talents = _userRepository.GetQueryable()
                                            .Skip(position)
                                            .Take(increment);
                ...
                foreach (var user in talents)
                {
                    ...
                    talentList.Add(new TalentSnapshotViewModel
                    {
                        Id = user.Id,
                        Name = user.FirstName + " " + user.LastName,
                        Skills = user.Skills.Select(s => s.Skill).ToList(),
                        CurrentEmployment = company,
                        CVUrl = string.IsNullOrWhiteSpace(user.CvName) ? "" : await _fileService.GetFileURL(user.CvName, FileType.UserVideo),
                        PhotoId = user.ProfilePhotoUrl,
                        Summary = user.Summary,
                        Visa = user.VisaStatus,
                        VideoUrl = string.IsNullOrWhiteSpace(user.VideoName) ? "" : await _fileService.GetFileURL(user.VideoName, FileType.UserVideo),
                        Level = level
                    });
